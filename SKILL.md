...
**With-skill run** (use `sessions_spawn` with `runtime="subagent"`, `mode="run"`):
...
IMPORTANT: Read the skill at <path-to-skill>/SKILL.md FIRST. Then follow its instructions to complete the task.
```
...
4. **Launch the viewer** with both qualitative outputs and quantitative data:
   ...
   **On OpenClaw / headless environments** (preferred — use `--static` to generate a standalone HTML file):
   ```bash
...
If you started a viewer server (non-static mode), kill it when you're done:

```bash
kill $VIEWER_PID 2>/dev/null
```

(In static/OpenClaw mode, no server is running — skip this step.)
...
3. Write to a file (e.g., `<workspace>/eval_review_<skill-name>.html`). On desktop environments, open it directly. On headless/OpenClaw environments, share the file path with the user or upload via CDN.
4. The user can edit queries, toggle should-trigger, add/remove entries, then click "Export Eval Set"
5. The file downloads as `eval_set.json` — on desktop, check `~/Downloads/`; on OpenClaw, the user will share the file back or paste the JSON content.
...
Package the skill and share the `.skill` file with the user:
...
The core workflow (draft → test → review → improve → repeat) is the same everywhere. Here's what to adapt based on your platform:

**Subagents not available** (e.g. Claude.ai): Run test cases one at a time instead of in parallel. Skip baseline runs and quantitative benchmarking — focus on qualitative feedback. Present results directly in the conversation rather than through the browser viewer. Ask for feedback inline: "How does this look? Anything you'd change?"

**No browser or display** (headless / remote environments): Use `--static <output_path>` when launching the eval viewer to write a standalone HTML file instead of starting a local server. The user can then open it in their browser directly. Feedback will download as `feedback.json` when they click "Submit All Reviews" — read it from there.
...
When running on **OpenClaw**, follow these adaptations:

**Subagents**: OpenClaw supports subagents via `sessions_spawn`. When the skill says "spawn a subagent", use the `sessions_spawn` tool with `runtime="subagent"` and `mode="run"`. Example for running a test case:
...
For parallel runs (e.g. with_skill + baseline), spawn multiple subagents at once — OpenClaw handles concurrent execution. You will receive a completion notification when each finishes.

**Eval viewer**: Since OpenClaw typically runs in a headless environment, always use `--static`:
...
**Trigger testing** — `openclaw_run_eval.py` (two-step: prepare → spawn subagents → aggregate):
```bash
# Step 1: Prepare task prompts
python -m scripts.openclaw_run_eval \
  --eval-set <trigger_eval.json> \
  --skill-path <path-to-skill> \
  --output-dir <workspace>/trigger_verdicts \
  --runs-per-query 1 \
  prepare --tasks-output <workspace>/trigger_tasks.json

# Step 2: Spawn subagents to execute each task's "prompt" field,
#         writing verdicts to each task's "output_path"

# Step 3: Aggregate verdicts into results
python -m scripts.openclaw_run_eval \
  --eval-set <trigger_eval.json> \
  --skill-path <path-to-skill> \
  --output-dir <workspace>/trigger_verdicts \
  aggregate \
  --tasks-input <workspace>/trigger_tasks.json \
  --results-output <workspace>/eval_results.json
```

**Description improvement** — `openclaw_improve_description.py` (two-step: generate prompt → spawn subagent → parse response):
```bash
# Step 1: Generate improvement prompt
python -m scripts.openclaw_improve_description generate \
  --eval-results <workspace>/eval_results.json \
  --skill-path <path-to-skill> \
  --output-dir <workspace>/improve_workspace

# Step 2: Spawn a subagent with the prompt at improve_workspace/improve_prompt.md,
#         save response to improve_workspace/response.md

# Step 3: Parse the response
python -m scripts.openclaw_improve_description parse \
  --response-file <workspace>/improve_workspace/response.md \
  --output <workspace>/improved_description.json
```

These scripts produce the same output format as the originals, so all downstream tools (generate_report.py, etc.) work unchanged.

**Python dependencies**: Before running any Python scripts, ensure `pyyaml` is installed:
```bash
